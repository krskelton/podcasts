...
export default {
  ...
  methods: {
    passwordMeetsRequirements() {
      ...
      if (
        !capitalLetters.test(this.password) ||
        !specialChracters.test(this.password) ||
        ...
        this.passwordRequirementsErrorMessage =
          "Passwords must be at least 5 characters long and include at least one capital letter and one special character";
        return;
      }
      if (this.password != this.confirmPassword) {
        // if the password and confirmed password don't match, display error
        this.passwordMatchErrorMessage =
          "Those passwords don't match. Please re-enter.";
        return;
      }
    ...
    testDuplicateUser() {
      ...
      axios
        .post("/duplicate-user-test", { username: this.username })
        .then(resp => {
